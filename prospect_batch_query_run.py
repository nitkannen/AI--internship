...
# In[362]:
ENV_VARIABLE= str(os.environ.get('NODE_ENV'))

if(ENV_VARIABLE == "prodSing"):
    #pass
    client = MongoClient('hSource=admin&replicaSet=prosing-shared-cluster-shard-0&retryWrites=true&w=majority&ssl=true&ssl_cert_reqs=CERT_NONE')
    db = client['prodsing']
    fbusers = db['fbUser']
    
else:
    client =  MongoClient('net/?ssl=true&ssl_cert_reqs=CERT_NONE')
    db = client['gogaga']
    fbusers = db['fbUser']
...
threshold_score = 0 # Can be adjusted, optimal value 0.3 for five prospects or below
taste_threshold_score = 0.0 # Threshold can be modified
...
def list_of_liked_users(user_json): # This function returns a list of users the user has liked
                    ...
                    for a in (liked_list[i][0].get('userInterests')):
                        ...
                        if (a != []) and (a != {}):
                            ...
                            for key in a.keys():
                                content[key] = a[key]
                        #print(content)
                if liked_list[i][0].get('userInterests') != [] and liked_list[i][0].get('userInterests') != {}:
...
def getFilteredUsers(s,preferences,user_dict):
    ...
    #print(men,women) 
    #print(preferences['men'])
    if(not(men and women)):
    #which means either men or women  and not both
        if(men):
            gender = 'male'
        else:
            gender = 'female'
...
def filter_based_on_prospects_pref(prospect_preferences_dict,user_json):
    ...
    match = True        #final bool return whether to accept or not
    ...
    if(('men' in prospect_preferences_dict) and ('women' in prospect_preferences_dict)):
        ...
        women =  prospect_preferences_dict['women']
        men = False
    else:  ##ideally never comes here
        men = False
        women  = False
    #print(men,women,prospect_preferences_dict,' ',str(user_dict.get('gender')))
...
for i in range(iters+1):
    ...
    #print(len(final_reordered_threshold))                
